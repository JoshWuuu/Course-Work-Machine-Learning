...
def initial_state():
    ...
    # *** START CODE HERE ***
    # use a list, first element is beta, second element is corresponding x value
    state = []
    return state
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    if not bool(state):
        ...
        result = 0.0
        # keep updating the result by multipling each state key value and x_i
        for (beta, x) in state:
            result += beta * kernel(x, x_i)
        return sign(result)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    # update beta with new sample x and y
