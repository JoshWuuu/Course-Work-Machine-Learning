...
def init_centroids(num_clusters, image):
    ...
    centroids_init = np.random.randint(low=0, high=256, size=[num_clusters, image.shape[-1]])
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    # *** START YOUR CODE ***
    # make image from 3d to 2d
    image = image.reshape(-1, 3)
    for i in range(max_iter):
         # find the closest centroid to every pixel and append the closest centroid to array
        CentroidsAssign = []
        for row in image:
            smallestD = 0
            # loop throught all the centroid
            for c in range(centroids.shape[0]):
                if (np.linalg.norm(row - centroids[c, :]) < smallestD or smallestD == 0):
                    closestC = c
                    smallestD = np.linalg.norm(row - centroids[c,:])
            CentroidsAssign.append(closestC)
        # convert to array
        CentroidsAssign1 = np.array(CentroidsAssign)
        # update centroid
        for i in range(centroids.shape[0]):
            # update according to the centroid sequence
            centroids[i] = np.mean(image[CentroidsAssign1 == i], axis=0)
...
def update_image(image, centroids):
    ...
    # *** START YOUR CODE ***
    # similar to prev func but can't convert image to 2d since it will influence the output
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            smallestD = 0
            for c in range(centroids.shape[0]):
                if (np.linalg.norm(image[i,j,:] - centroids[c, :]) < smallestD or smallestD == 0):
                    closestC = c
                    smallestD = np.linalg.norm(image[i,j,:] - centroids[c,:])
            image[i,j,:] = centroids[closestC]
